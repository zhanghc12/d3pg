...
class EnsembleDynamicsModel():
    def __init__(self, network_size, elite_size, state_size, action_size, reward_size=1, hidden_size=200, use_decay=True, env_name='', inner_epoch_num=5, use_disentangle=False):
    ...
    def train(self, inputs, labels, batch_size=256, holdout_ratio=0., max_epochs_since_update=5):
        ...
        for epoch in range(self.inner_epoch_num):
            ...
            for start_pos in range(0, train_inputs.shape[0], batch_size):
                ...
                losses = []
                mean, logvar, sa_output, s_output = self.ensemble_model(train_input, ret_log_var=True)
                loss, _ = self.ensemble_model.loss(mean, logvar, train_label, sa_output, s_output)
                self.ensemble_model.train(loss)
                losses.append(loss)

            with torch.no_grad():
                holdout_mean, holdout_logvar, holdout_sa_output, holdout_s_output = self.ensemble_model(holdout_inputs, ret_log_var=True)
