    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        if self.total_it % 100 == 0:
            ...
            with torch.no_grad():
                ...
                test_fixed_target_Q1_0 = test_noisy_target_Q1_0 + torch.norm(next_state_grad[:batch_size], dim=1, keepdim=True) * 1e-4 * 0.03  # torch.sum(next_state_grad[:batch_size] * (perturbed_next_state_0 - next_state), dim=1, keepdim=True)
                test_fixed_target_Q1_1 = test_noisy_target_Q1_1 + torch.norm(next_state_grad[batch_size:2*batch_size], dim=1, keepdim=True) * 1e-3 * 0.03  #  torch.sum(next_state_grad[batch_size:2*batch_size] * (perturbed_next_state_1 - next_state), dim=1, keepdim=True)
                test_fixed_target_Q1_2 = test_noisy_target_Q1_2 + torch.norm(next_state_grad[2*batch_size:3*batch_size], dim=1, keepdim=True) * 3e-2 * 0.03  # torch.sum(next_state_grad[2*batch_size:3*batch_size] * (perturbed_next_state_2 - next_state), dim=1, keepdim=True)
                test_fixed_target_Q1_3 = test_noisy_target_Q1_3 + torch.norm(next_state_grad[3*batch_size:], dim=1, keepdim=True) * 1e-1 * 0.03  # torch.sum(next_state_grad[3*batch_size:] * (perturbed_next_state_3 - next_state), dim=1, keepdim=True)
