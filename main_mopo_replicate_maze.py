...
def load(dataset, replay_buffer):
    obs_mean = np.mean(dataset.item()['observations'], axis=0, keepdims=True)
    obs_std = np.std(dataset.item()['observations'], axis=0, keepdims=True)
    replay_buffer.state = normalize(dataset.item()['observations'])
    # replay_buffer.action = normalize(dataset['actions'])
    replay_buffer.action = dataset.item()['actions']

    replay_buffer.next_state = normalize(dataset.item()['next_observations'])
    replay_buffer.reward = np.expand_dims(np.squeeze(dataset.item()['rewards']), 1)
    replay_buffer.not_done = 1 - np.expand_dims(np.squeeze(dataset.item()['terminals']), 1)
    replay_buffer.next_action = np.concatenate([dataset.item()['actions'][1:],dataset.item()['actions'][-1:]], axis=0)
    replay_buffer.forward_label = normalize(np.concatenate([dataset.item()['next_observations'] - dataset.item()['observations'], np.expand_dims(np.squeeze(dataset.item()['rewards']), 1)], axis=1))

    replay_buffer.size = dataset.item()['terminals'].shape[0]
