...
if True:
    ...
    # plt.figure()
    axes[0].contourf(x_lin, y_lin, z, 10, cmap='cividis')

    data = np.concatenate([post_observations, actions], axis=1)
    np.random.shuffle(data)
    axes[0].scatter(data[:5000, 0], data[:5000, 1], marker='.', s=20, c='#EA7F45')#marker='x')# , s=12)
    axes[0].set_title('MOPO-style Ensemble', fontsize=fontsize)
...
if True:
    ...
    # plt.figure()
    axes[1].contourf(x_lin, y_lin, z, 4, cmap='cividis')

    data = np.concatenate([post_observations, actions], axis=1)
    np.random.shuffle(data)
    axes[1].scatter(data[:5000, 0], data[:5000, 1], marker='.', s=20, c='#EA7F45')#marker='x')# , s=12)
    axes[1].set_title('Standard Model Ensemble', fontsize=fontsize)
