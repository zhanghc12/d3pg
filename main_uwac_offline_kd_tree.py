...
def predict_uncertainty(qf1, qf2, vae, state_batch, action_batch, tree):
    state_batch = torch.FloatTensor(state_batch).to(device=device)
    action_batch = torch.FloatTensor(action_batch).to(device=device)

    ood_data1 = qf1.get_feature(state_batch, action_batch).detach().cpu().numpy()

    weight = tree.query(ood_data1, k=1)[0][:, 1]
    # now put vae forward
    # print(weight.shape)
...
def test_uncertainty(memory, qf1, qf2, vae_policy, batch_size=2560):
    ...
    while i + batch_size < memory.size:
        ...
        phi = qf1.get_feature(state_batch, action_batch)
    ...
    while i + batch_size < size:
        ...
        iid_distance = predict_uncertainty(qf1, qf2, vae_policy, state_batch, action_batch)
        ood_distance1 = predict_uncertainty(qf1, qf2, vae_policy, state_batch, ood_action_batch1)
        ood_distance2 = predict_uncertainty(qf1, qf2, vae_policy, state_batch, ood_action_batch2)
        ood_distance3 = predict_uncertainty(qf1, qf2, vae_policy, state_batch, ood_action_batch3)
        ood_distance4 = predict_uncertainty(qf1, qf2, vae_policy, state_batch, ood_action_batch4)
...
def main(args=None):
    ...
    test_uncertainty(replay_buffer, qf1, qf2, vae_policy)
